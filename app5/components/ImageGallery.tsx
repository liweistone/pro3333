        ...
        {items.map((item) => (
          ...
          >
                ...
                <div className="flex flex-col items-center gap-4 w-full px-8 text-center">
                  ...
                  <div className="w-full">
                     <p className="text-xs font-bold text-slate-700 mb-1">
                       {item.status === 'pending' ? '任务正在排队...' : 'AI 深度创作中...'}
                     </p>
                     ...
                     <p className="mt-2 text-[10px] text-slate-400 font-mono">ID: {item.taskId?.slice(0, 8) || '等待中...'}</p>
                  </div>
                </div>
              )}
            ...
            <div className="p-4 flex-1 flex flex-col bg-white">
              <p className="text-[11px] text-slate-600 line-clamp-2 font-medium flex-1 italic leading-relaxed cursor-pointer hover:text-blue-600" onClick={() => item.status === 'succeeded' && setPreviewItem(item)}>
                "{item.prompt}"
              </p>
